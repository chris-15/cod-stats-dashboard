...
export function MobileNav({ game: game = "bo6" }: MobileNavProps) {
  ...
  return status === "authenticated" ? (
    <nav
      className={`h-16 px-4 pt-2 text-gray-400 bg-background transition-all duration-300 ${
        isVisible ? "translate-y-0 opacity-100" : "translate-y-full opacity-0"
      }`}
    >
      <div className="flex justify-around items-center bg-[hsl(var(--sidebar-background))] h-12 rounded-lg">
